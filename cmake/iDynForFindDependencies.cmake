...
#
# IDYNFOR_USE_${Package}: Can be disabled by the user if he doesn't want to use that
#                      dependency.
# IDYNFOR_HAS_${Package}: Internal flag. It should be used to check if a part of
#                      IDYNFOR should be built. It is on if IDYNFOR_USE_${Package} is
#                      on and either the package was found or will be built.
# IDYNFOR_BUILD_${Package}: Internal flag. Used to check if IDYNFOR has to build an
#                        external package.
# IDYNFOR_BUILD_DEPS_${Package}: Internal flag. Used to check if dependencies
#                             required to build the package are available.
# IDYNFOR_HAS_SYSTEM_${Package}: Internal flag. Used to check if the package is
#                             available on the system.
# IDYNFOR_USE_SYSTEM_${Package}: This flag is shown only for packages in the
...
#                             the version shipped within the IDYNFOR.
...
macro(IDYNFOR_dependent_option _option _doc _default _deps _force)
  ...
  if(DEFINED ${_option})
    ...
    if(_option_strings_set)
      ...
      if("${${_option}}" STREQUAL "${_option_strings_last}")
        message(SEND_ERROR "\"${_option_strings_first}\" is needed to enable ${_option}.")
      endif()
      unset(${_option} CACHE)
    endif()
  endif()
